...
export async function searchHandler(params: SearchParams) {
  ...
  let restaurantVariants = params.restaurantVariants;
  ...
  // If we have explicitRestaurantQuery, resolve it against database using universal resolver
  if (explicitRestaurantQuery && !restaurantNameFromSearchKey) {
    ...
    if (resolution.status === 'MATCH') {
      ...
      if (resolution.variants && resolution.variants.length > 0) {
        restaurantVariants = resolution.variants;
      }

      console.log('[searchHandler] Restaurant resolved (universal):', {
        name: restaurantName,
        id: restaurantId,
        matchType: resolution.matchType,
        variants: restaurantVariants?.length
      });
    } else if (resolution.status === 'AMBIGUOUS') {
      console.log('[searchHandler] Restaurant AMBIGUOUS:', resolution.candidates);
      const candidatesList = resolution.candidates.map(c => c.name).join(', ');
      return {
        meals: [],
        hasMore: false,
        nextOffset: 0,
        searchKey: currentSearchKey || '',
        message: `Multiple restaurants found for "${explicitRestaurantQuery}": ${candidatesList}. Please be more specific.`
      };
    } else {
      // NOT_FOUND, NO_RESTAURANT, NO_MEALS
      console.log('[searchHandler] Restaurant lookup failed:', resolution.status);
      return {
        meals: [],
        hasMore: false,
        nextOffset: 0,
