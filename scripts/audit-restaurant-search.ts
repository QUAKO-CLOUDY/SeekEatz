...
function auditDuplicateRestaurantNames(rows: MenuItemRestaurantRow[]) {
  ...
  for (const row of rows) {
    ...
    const key = normalizeName(row.restaurant_name);
    const variants = grouped.get(key) ?? new Set<string>();
    variants.add(row.restaurant_name);
    grouped.set(key, variants);
  }

  return Array.from(grouped.entries())
    .filter(([, variants]) => variants.size > 1)
    .map(([key, variants]) => ({ key, variants: Array.from(variants).sort() }))
    .sort((a, b) => a.key.localeCompare(b.key));
}

async function auditRestaurantQueries(supabase: ReturnType<typeof createSupabaseClient>, restaurantNames: string[]) {
...
async function main() {
  ...
  const duplicateGroups = auditDuplicateRestaurantNames(allRows);
  ...
  console.log(`Duplicate normalized restaurant_name groups: ${duplicateGroups.length}`);
  for (const group of duplicateGroups) {
    console.log(`DUPLICATE ${group.key}: ${group.variants.join(' | ')}`);
  }
...
main().catch((error) => {
  console.error('[audit-restaurant-search] fatal error:', error);
  process.exit(1);
});
