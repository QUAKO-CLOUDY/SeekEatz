...
import { MessageSquare, BarChart3, Settings, Heart, Send, ArrowRight, Lock, Clock } from 'lucide-react';
...
function MockChat() {
    return (
        <div className="flex flex-col h-full bg-white rounded-b-2xl">
            ...
            {/* Messages */}
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 space-y-3 sm:space-y-4">
                {/* AI message */}
                        ...
                        {[
                            ...
                            <div key={i} className="bg-white border border-gray-100 rounded-xl p-2.5 shadow-sm">
                                ...
                                <div className="flex gap-3">
                                    <span className="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{meal.protein}g</span>
...
function MockDailyLog() {
    ...
    const ringProgress = [
        { key: 'calories', current: eaten, target: goal, colorStart: '#f472b6', colorEnd: '#f43f5e', size: 104, stroke: 8 },
        { key: 'protein', current: 98, target: 150, colorStart: '#22d3ee', colorEnd: '#3b82f6', size: 88, stroke: 7 },
        { key: 'carbs', current: 140, target: 200, colorStart: '#4ade80', colorEnd: '#10b981', size: 72, stroke: 6 },
        { key: 'fats', current: 42, target: 65, colorStart: '#fbbf24', colorEnd: '#f97316', size: 56, stroke: 5 },
    ] as const;
    ...
    return (
        <div className="flex flex-col h-full bg-white rounded-b-2xl">
            ...
            <div className="flex-1 overflow-hidden px-5 py-4 space-y-5">
                {/* Stacked rings + remaining */}
                <div className="flex items-center gap-5">
                    <div className="relative h-28 w-28 flex-shrink-0">
                        {ringProgress.map((ring) => {
                            const percentage = Math.max(0, Math.min(100, Math.round((ring.current / ring.target) * 100)));
                            const radius = (ring.size - ring.stroke) / 2;
                            const circumference = radius * 2 * Math.PI;
                            const gradientId = `preview-log-${ring.key}`;
                            ...
                            return (
                                ...
                                >
                                    <svg viewBox={`0 0 ${ring.size} ${ring.size}`} className="h-full w-full -rotate-90">
                                        <defs>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                            <span className="text-base font-bold text-gray-900">{goal}</span>
                        </div>
                    </div>
                    <div className="space-y-2 flex-1">
                        <div className="flex justify-between text-xs">
                            <span className="text-gray-500">Remaining Macros</span>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                    {[
                        { label: 'Calories', current: eaten, target: goal, bg: 'bg-pink-50', text: 'text-pink-700', suffix: '' },
                        { label: 'Protein', current: 98, target: 150, bg: 'bg-cyan-50', text: 'text-cyan-700', suffix: 'g' },
                        { label: 'Carbs', current: 140, target: 200, bg: 'bg-green-50', text: 'text-green-700', suffix: 'g' },
                        { label: 'Fats', current: 42, target: 65, bg: 'bg-amber-50', text: 'text-amber-700', suffix: 'g' },
                    ].map((macro) => (
                        <div key={macro.label} className={`${macro.bg} rounded-xl p-3 text-center`}>
                            <p className={`text-lg font-bold ${macro.text}`}>{macro.current}{macro.suffix}</p>
                            <p className="text-[10px] text-gray-400">/ {macro.target}{macro.suffix}</p>
                            <p className="text-[10px] font-medium text-gray-500 mt-1">{macro.label}</p>
                        </div>
                    ))}
                </div>
...
function MockFavorites() {
    ...
    const recentMeals = [
        ...
        { name: 'Poke Bowl', restaurant: 'Pokeworks', cal: 520, protein: 38, time: 'Yesterday' },
                            ...
                            <div key={i} className="flex items-center gap-3 p-3 bg-gray-50/80 rounded-xl border border-gray-100/60 hover:border-pink-200 transition-colors cursor-pointer group">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-3 mt-1.5">
                                        <span className="text-xs text-gray-500">{meal.cal}</span>
                                        <span className="text-xs text-gray-500">{meal.protein}g</span>
            ...
            <div className="max-w-5xl mx-auto">
                ...
                >
                    ...
                    <div className="relative overflow-hidden rounded-2xl border border-gray-200/60 bg-white shadow-xl shadow-gray-900/10">
                        ...
                        {/* Screen content */}
                        <div className="h-[520px] overflow-hidden sm:h-[560px] lg:h-[600px]">
                        ...
                        <div className="absolute bottom-0 inset-x-0 h-24 sm:h-32 bg-gradient-to-t from-white via-white/90 to-transparent pointer-events-none" />

                        {/* CTA overlay */}
                        <div className="absolute bottom-4 sm:bottom-6 inset-x-0 z-40 flex justify-center pointer-events-auto px-4">
                            <Link href="/auth/signup">
                                <Button className="rounded-full bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-white px-6 sm:px-8 py-4 sm:py-5 text-xs sm:text-sm font-bold shadow-xl shadow-cyan-500/25 hover:shadow-cyan-500/35 transition-all hover:-translate-y-0.5 gap-2">
