-- Migration 010: Meal cards must only show item_type='meal'
-- Also fix search_restaurants_trgm overload ambiguity

-- ── 1. Update search_menu_items to enforce item_type = 'meal' only ────────────
...
-- ── 2. Fix search_restaurants_trgm ambiguity ─────────────────────────────────

DROP FUNCTION IF EXISTS search_restaurants_trgm(text);
DROP FUNCTION IF EXISTS search_restaurants_trgm(text, integer);

CREATE FUNCTION search_restaurants_trgm(query_text text)
RETURNS TABLE (name text, score real)
LANGUAGE SQL STABLE AS $$
  SELECT DISTINCT ON (r.name)
    r.name,
    similarity(LOWER(r.name), LOWER(query_text)) AS score
  FROM restaurants r
  WHERE
    similarity(LOWER(r.name), LOWER(query_text)) > 0.15
    OR LOWER(r.name) ILIKE '%' || LOWER(query_text) || '%'
  ORDER BY r.name, score DESC
  LIMIT 10;
$$;

GRANT EXECUTE ON FUNCTION search_restaurants_trgm TO anon, authenticated;
